...
バックエンドには Firebase を利用しており、Firebase Emulator を用いたローカル開発環境、自動テスト、CI/CD の構築など、DevOps 的な作業も積極的に行いました。用意した環境とツールセットはチームメンバーによく受け入れられ、開発が快適になったと感謝の声を貰いました。

テストについては初めは全く用意されていない状態でしたが、自らテスト基盤の用意を進め、最終的にはカバレッジ 80%以上を達成しました。この取り組みと成果はのちに社内で発表を行い、社内のテスト文化普及に貢献しました。
